...
public class FastPoolManager : MonoBehaviour
{
    ...
    [SerializeField] List<FastPool> predefinedPools;
    Dictionary<int, FastPool> pools;

    UnityEngine.Transform curTransform;
            ...
            {
                {
                    if (!pools.ContainsKey(predefinedPools[i].ID))
                        ...
                        Debug.LogError(
                            "FastPoolManager has a several pools with the same ID. Please make sure that all your pools have unique IDs");
                }
            }
    ...
    public static FastPool CreatePoolC<T>(T component, bool warmOnLoad = true, int preloadCount = 0,
        int capacity = 0)
        where T : Component
    {
        if (component != null)
    ...
    /// <param name="warmOnLoad">Load source prefab in the memory while scene is loading. A bit slower scene loading, but much faster instantiating of new objects in pool</param>
    /// <returns>FastPool instance</returns>
    public static FastPool CreatePool(GameObject prefab, bool warmOnLoad = true, int preloadCount = 0,
        int capacity = 0)
    {
        if (prefab != null)
        {
    ...
    /// <param name="warmOnLoad">Load source prefab in the memory while scene is loading. A bit slower scene loading, but much faster instantiating of new objects in pool</param>
    /// <returns>FastPool instance</returns>
    public static FastPool CreatePool(int id, GameObject prefab, bool warmOnLoad = true, int preloadCount = 0,
        int capacity = 0)
    {
        if (prefab != null)
        {
            ...
            {
                FastPool newPool = new FastPool(id, prefab, Instance.curTransform, warmOnLoad, preloadCount,
                    capacity);
    ...
    public static FastPool GetPool(GameObject prefab, bool createIfNotExists = true)
    {
        if (prefab != null)
        {
            if (Instance.pools.ContainsKey(prefab.GetInstanceID()))
                ...
                return CreatePool(prefab, true, 0, 0);
        }
        else
        {
    ...
    public static FastPool GetPool(GameObject prefab, int preloadCount, bool createIfNotExists = true)
    {
        if (prefab != null)
        {
            if (Instance.pools.ContainsKey(prefab.GetInstanceID()))
                ...
                return CreatePool(prefab, true, preloadCount, 0);
        }
        else
        {
            Debug.LogError("Trying to get pool for null object");
    ...
    public static void CreatPoolIfNotExisted(GameObject prefab, bool warmOnLoad = true, int preloadCount = 0,
        int capacity = 0)
    {
        if (prefab != null)
        {
            if (!Instance.pools.ContainsKey(prefab.GetInstanceID()))
            {
                CreatePool(prefab, true, preloadCount, capacity);
            }
        }
    }
    ...
    public IEnumerator CoDestroyPoolSlowly()
    {
        ...
        {
            yield return StartCoroutine(pool.Value.CoClearCache());
        }
    }
}
